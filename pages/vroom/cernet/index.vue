<template>
  <div>
      ...
      <c-header :showNav="false">
        ...
        <template v-slot:body>
          <div class="header-body" data-slide-up="true">
            ...
            <div class="company-name slide-up delay-5">{{ header.companyName }}</div>
            <div class="company-desc slide-up delay-8">{{ header.companyDesc }}</div>
            <a class="play-btn slide-up delay-10" @click="openVideoModal(header.originVideo)">
              <div class="icon-play"></div>观看视频
            </a>
          </div>
        </template>
      </c-header>
    ...
    <div class="content">
      ...
      <div v-if="curTab.id === 'achieve'" class="achieve">
        ...
        <section class="cbox cbox-video" data-slide-up="true">
          ...
          <div class="cbox-bd clearfix slide-up delay-5">
            <div v-for="(videoItem, vidindex) in videosList" :key="vidindex" class="vbox">
      ...
      <div v-else class="about">
        <p v-for="(intro, introIndex) in about.introduction" :key="introIndex">{{ intro }}</p>
        <div class="pic-wrap">
          <img v-for="(pic, index) in about.booths" :key="index" :src="pic.picUrl" />
        </div>
    ...
    />
    <videos-modal v-model="showVideosModal" :url="curVideoUrl" :type="curVideoType" />
  </div>
</template>
